...
export class StorageWrapper<T extends SchemaDefinition> {
    ...
    for (const key in this.schema) {
      if (data[key] !== undefined) {
        ...
        if (fieldDef.type === "array" && Array.isArray(data[key])) {
          ...
          const arrayValues = (data[key] as (string | number)[]).map((item) =>
            String(item).replaceAll(arraySep, `\\${arraySep}`),
          );
        ...
        encodedValue = encodedValue.replaceAll(this.separator, `\\${this.separator}`);
      ...
      switch (fieldType) {
        ...
        case "array": {
          ...
          if (!value) {
            ...
            result[key] = arrayType === "number" ? items.map((item) => Number(item)) : items;
          }
          break;
        }
        case "string":
  ...
  get(): DataFromSchema<T> | null {
    ...
    try {
      ...
      console.error(`Failed to decode storage item with key "${key}":`, error);
      return null;
    }
  }
...
export class StorageManager {
  ...
  private static getInstanceKey(schemaKey: SchemaKey, params?: Record<string, string>): string {
  ...
  static get<T extends SchemaKey>(
    ...
    if (!this.instances.has(instanceKey)) {
      const config = STORAGE_SCHEMAS[schemaKey] as KeyConfig<SchemaDefinition>;
...
export { createDynamicKey, createField, createKeyPattern } from "./storage-helpers";

export function useStorage<T extends SchemaKey>(schemaKey: T, params?: Record<string, string>) {
