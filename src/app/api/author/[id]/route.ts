...
import { SmallManga, SimpleError, MangaListResponse } from "../../interfaces";
...
export async function fetchMangaAuthor(
    authorId: string,
    orderBy: string,
    page: number,
    ...
    const cachedData: MangaListResponse | undefined = cache.get(cacheKey);
    if (cachedData) {
        return cachedData;
    }

    if (!authorId) {
        return { result: "error", data: "Author ID is required" };
    }
...
export async function GET(
    ...
    try {
        ...
        const result = await fetchMangaAuthor(authorId, orderBy, Number(page));
