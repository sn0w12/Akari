...
export async function POST(request: NextRequest) {
    try {
        const { username, password, captcha, ciSessionCookie } =
            await request.json();
        ...
        const redirectResponse = await client.get(
        ...
        const scriptSrc = $(
            'script[src^="https://manganelo.com/login_al"]',
        ).attr("src");
        if (!scriptSrc) {
            return NextResponse.json(
        ...
        const userAccCookie = setCookieHeader.find((cookie) =>
            cookie.startsWith("user_acc="),
        );
        if (userAccCookie) {
