...
export function useBodyScrollListener(
  ...
  useEffect(() => {
    ...
    const mainElement = document.getElementById("scroll-element") as HTMLElement;
    ...
    const initialElement =
      initialMainScroll >= initialWindowScroll ? mainElement : document.documentElement;
    callbackRef.current(initialElement);

    const wrappedCallback = () => {
      const mainScroll = mainElement.scrollTop;
      const windowScroll = window.scrollY;
      const element = mainScroll >= windowScroll ? mainElement : document.documentElement;
