...
function getLongPressOptions(options?: Partial<LongPressOptions>): LongPressOptions {
  return {
...
export function useLongPress(
  ...
  const stopTimer = useCallback(
    (fromUser = false) => {
      ...
      if (!(fromUser && opt.controlledAfterPress && longPressed.current)) {
        setPressed(false);
      }
      if (timerRef.current) {
  ...
  useBodyScrollListener(
    (element) => {
      ...
      if (Math.abs(current - initialScroll.current) > SCROLL_CANCEL_THRESHOLD) {
        stopTimer(true);
      }
    },
    { passive: true },
  ...
  const transitionDuration = pressed ? delay - opt.scaleDelay : opt.releaseDuration;
