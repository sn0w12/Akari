...
export const Route = createFileRoute("/_default/genre/$id/$page/")({
    validateSearch: (search: Record<string, string | undefined>) => ({
        sort: (search.sort ?? "latest") as "search" | "latest" | "popular" | "newest" | undefined,
    }),
    loaderDeps: ({ search }) => ({ sort: search.sort ?? "latest" as string }),
    loader: async ({ params, deps }) => {
        const name = decodeURIComponent(params.id).replaceAll("-", " ");
        const page = Number(params.page) || 1;
        return getGenreData({ data: { name, page, sort: deps.sort as "search" | "latest" | "popular" | "newest" } });
    },
    head: ({ loaderData, params }) => {
        ...
        return createMetadata({
            title: `${name} Manga - Page ${page}`,
            description: `Browse manga in the ${name} genre on Akari.`,
            canonicalPath: `/genre/${params.id}/${page}`,
            image: createOgImage("genre", params.id),
            pagination: {
                ...(page > 1 ? { previous: `/genre/${params.id}/${page - 1}` } : {}),
                ...(totalPages && page < totalPages ? { next: `/genre/${params.id}/${page + 1}` } : {}),
            },
        });
    },
    component: GenrePaginatedPage,
});

function GenrePaginatedPage() {
