...
export const Route = createFileRoute("/_default/sync/mal/")({
  ...
  headers: () => ({
    "Cache-Control": new ResponseCacheControlBuilder().noCache().private().build(),
  }),
});

function SyncMalPage() {
  const [{ malData, bookmarks, malLoading, bookmarksLoading, bookmarksProgress }, dispatch] =
  ...
  async function syncMalToBookmarks() {
    ...
    const confirmed = await confirm({
      ...
      description: "Are you sure you want to sync your MAL data to bookmarks?",
    ...
    toastManager.add({
      ...
      type: "info",
      description: alreadySynced > 0 ? `Skipping ${alreadySynced} already synced manga` : undefined,
    });
    ...
    const malDataById = new Map(malData.map((item) => [item.node.id, item]));
    const malDataToSyncById = new Map(malDataToSync.map((item) => [item.node.id, item]));

    const batchPromises = [];
    for (let i = 0; i < malDataToSync.length; i += batchSize) {
      const batchIds = malDataToSync.slice(i, i + batchSize).map((item) => item.node.id);
  ...
  const renderRow = (item: components["schemas"]["MalMangaListItem"]) => (
    <TableRow key={item.node.id}>
      <TableCell className="font-medium max-w-xs truncate">
        <Link className="hover:underline" to="/mal/$malId" params={{ malId: String(item.node.id) }}>
          {item.node.title || "Unknown Title"}
        </Link>
      </TableCell>
      <TableCell>{item.listStatus?.numChaptersRead}</TableCell>
      <TableCell className="min-w-[100px]">
        <Badge variant={getStatusVariant(item.listStatus?.status || null)}>
