...
function SyncAniPage() {
  const [{ aniData, bookmarks, aniLoading, bookmarksLoading, bookmarksProgress }, dispatch] =
  ...
  async function syncAniToBookmarks() {
    ...
    const confirmed = await confirm({
      ...
      description: "Are you sure you want to sync your AniList data to bookmarks?",
    ...
    const aniDataById = new Map(aniData.map((item) => [item.media.id, item]));
    const aniDataToSyncById = new Map(aniDataToSync.map((item) => [item.media.id, item]));

    const batchPromises = [];
    for (let i = 0; i < aniDataToSync.length; i += batchSize) {
      const batchIds = aniDataToSync.slice(i, i + batchSize).map((item) => item.media.id);
