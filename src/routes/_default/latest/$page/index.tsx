...
export const Route = createFileRoute("/_default/latest/$page/")({
    loader: async ({ params }) => getLatestData({ data: { page: Number(params.page) || 1 } }),
    head: ({ loaderData, params }) => {
        const page = Number(params.page) || 1;
        const totalPages = loaderData?.data?.data?.totalPages;
        return createMetadata({
            title: `Latest Releases - Page ${page}`,
            description: "Read the latest manga releases for free on Akari.",
            canonicalPath: `/latest/${page}`,
            image: "/og/akari.webp",
            pagination: {
                ...(page > 1 ? { previous: `/latest/${page - 1}` } : {}),
                ...(totalPages && page < totalPages ? { next: `/latest/${page + 1}` } : {}),
            },
        });
    },
