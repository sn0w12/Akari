                    ...
                    <Link key={user.userId} to="/user/$userId" params={{ userId: user.userId }}>
                        <Card className="gap-0 py-4 hover:bg-accent transition-colors">
                            <CardHeader className="flex flex-row items-center gap-1 px-4">
                                ...
                                {user.banned ? <Badge className="self-center" variant="destructive">Banned</Badge> : null}
                                {user.role !== "user" ? (
                            ...
                            <CardContent className="px-4">
                                <p className="text-sm text-muted-foreground">{user.totalBookmarks} bookmarks, {user.totalComments} comments</p>
                            </CardContent>
                        </Card>
                    </Link>
                ))}
