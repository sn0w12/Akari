...
const loadUserPage = createServerFn({ method: "GET" })
    ...
    .handler(async ({ data: userId }) => {
        const { data: userResult, error: userError } = await client.GET("/v2/user/{userId}", {
            params: { path: { userId } },
            headers: serverHeaders,
        });

        const { data: listsData } = await client.GET("/v2/lists/user/{userId}", {
            params: { path: { userId }, query: { page: 1, pageSize: 12 } },
            headers: serverHeaders,
        });

        return { user: userResult?.data ?? null, userError, lists: listsData?.data ?? null };
    });
            ...
            <div className="flex flex-row justify-between items-center">
                <div className="flex flex-col md:flex-row items-baseline md:gap-2">
                    <div className="flex flex-row gap-2 items-baseline">
                        ...
                        <h2 className="text-4xl font-bold">{user.displayName || user.username}</h2>
                    </div>
                    <div className="flex flex-row gap-2 items-baseline">
                        <p className="text-muted-foreground leading-right">@{user.username}</p>
                        {user.banned ? <Badge variant="destructive">Banned</Badge> : null}
                        {user.role !== "user" ? (
                            <Badge variant={ROLE_VARIANT_MAP[user.role]}>{capitalize(user.role)}</Badge>
                        ) : null}
                    </div>
                </div>
                <p className="text-foreground/75 text-sm">
                    {user.totalBookmarks} bookmarks &bull; {user.totalComments} comments
                </p>
            </div>
            <Separator className="my-2" />
            <Suspense fallback={<UserListsSkeleton />}>
                <UserLists userId={id} initialData={{ result: "Success" as const, status: 200 as const, data: lists ?? { items: [], totalItems: 0, currentPage: 1, pageSize: 12, totalPages: 0 } }} />
            </Suspense>
        </div>
    );
}
