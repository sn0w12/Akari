    ...
    server: {
        handlers: {
            POST: async ({ request }) => {
                ...
                const invalidPages = !pages || !Array.isArray(pages) || pages.length === 0 || !pages.every((page) => typeof page === "string" && page.trim() !== "" && page.startsWith("/"));
                const invalidTags = !tags || !Array.isArray(tags) || tags.length === 0;

                if (invalidPages || invalidTags) {
                    return new Response(JSON.stringify({ message: "Missing or invalid pages or tags parameter" }), { status: 400 });
                }
