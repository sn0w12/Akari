...
function Entry({
  ...
  return (
    <Card ref={setNodeRef} style={style} className="flex flex-row items-center gap-2 p-2">
      {dragHandle && (
        <div className="self-stretch flex items-center" {...dragProps}>
          {dragHandle}
        </div>
      )}
      <Link to="/manga/$mangaId" params={{ mangaId: entry.mangaId }} className="shrink-0">
        <Image
      ...
      <div className="flex-1 min-w-0">
        <Link to="/manga/$mangaId" params={{ mangaId: entry.mangaId }}>
          <h3 className="font-semibold truncate hover:underline">{entry.mangaTitle}</h3>
        </Link>
        <p className="text-sm text-muted-foreground line-clamp-2">{entry.mangaDescription}</p>
      </div>
      {userId === ownerId && (
...
export function ListComponent({ id }: { id: string }) {
  ...
  async function persistEntryOrder({
    ...
    if (error) {
      ...
      toastManager.add({
        title: error.data?.message || "Failed to update entry order",
        type: "error",
      });
      return;
    }
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    const currentIndex = data.entries.findIndex((item) => String(item.id) === String(active.id));
    const newIndex = data.entries.findIndex((item) => String(item.id) === String(over.id));
    ...
    queryClient.setQueryData(["list", id], (currentData: typeof data) => {
      ...
      const nextEntries = arrayMove(currentData.entries, currentIndex, newIndex);
      return {
        ...currentData,
        entries: nextEntries,
      };
