...
export default function PageReader({
  ...
  const [currentPage, setCurrentPage] = useState(() => {
    ...
    return isNaN(pageParam) || pageParam < 1 || pageParam > chapter.images.length
      ? 0
      : pageParam - 1;
  });
  ...
  useEffect(() => {
    ...
    if (nextChapter && !hasPrefetchedRef.current) {
      const threshold = Math.min(Math.floor(imagesLength * 0.75), imagesLength - 3);
  ...
  const nextPage = useCallback(() => {
    if (currentPage === chapter.images.length - 1 && chapter.nextChapter && continueAfterChapter) {
  ...
  return (
    <>
      <ChapterInfo chapter={chapter} scanlator={scanlator} hidden={scrollMetrics.pixels >= 50} />
      <div className="w-full h-full flex flex-col relative" style={{ height: pageHeightStyle }}>
            ...
            {chapter.images[currentPage] && (
              <Image
                ...
                className={cn("w-full h-auto object-contain", {
                  "cursor-none": isInactive && currentPage !== chapter.images.length,
                  "cursor-pointer": !isInactive && currentPage !== chapter.images.length,
                })}
                style={{
      ...
      <div
        className={`sm:opacity-0 lg:opacity-100 transition-opacity duration-300 ${
          currentPage !== chapter.images.length ? "block" : "hidden md:block"
        }`}
      >
        <PageProgress
          currentPage={Math.max(0, Math.min(currentPage, chapter.images.length - 1))}
