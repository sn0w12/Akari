...
export default function EndOfManga({ title, identifier, style, className = "" }: EndOfMangaProps) {
  ...
  return (
    ...
    >
      <CardHeader>
        <CardTitle className="text-2xl md:text-3xl text-center">{title}</CardTitle>
      </CardHeader>
      <CardContent className="flex flex-col items-center justify-center flex-grow gap-y-6 text-center px-4">
        ...
        <p className="text-muted-foreground">
          This is the final chapter currently released. Check back later for updates!
        </p>
      </CardContent>
      <CardFooter className="flex flex-col gap-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 w-full">
          ...
          <Button
            variant="outline"
            render={<Link to="/manga/$mangaId" params={{ mangaId: identifier }} />}
