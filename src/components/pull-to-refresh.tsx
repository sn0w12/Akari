...
export function PullToRefresh({
  ...
  useEffect(() => {
    ...
    document.documentElement.style.overscrollBehavior =
      isEnabled && isAtTopRef.current && canPullRef.current ? "none" : "auto";
  }, [isEnabled]);
  ...
  const handleTouchStart = useCallback(
    (event: React.TouchEvent<HTMLElement>): void => {
      ...
      if (typeof document !== "undefined" && document.querySelector("[data-base-ui-focus-guard]"))
        return;
  ...
  return (
    <>
      <div
        className={cn(
          "pointer-events-none absolute left-0 right-0 top-0 z-10 flex h-12 items-center justify-center text-muted-foreground transition-opacity mt-[var(--safe-top)]",
      ...
      >
        <div className="flex items-center gap-2 text-xs">
          ...
          {(pullDistance > 0 || isRefreshing) && (
            <span>{isRefreshing ? refreshingText : progress >= 1 ? releaseText : pullText}</span>
          )}
        </div>
      </div>
      <Component
