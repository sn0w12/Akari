...
function findDuplicateShortcuts(settingsMap: Record<string, Setting>): Set<string> {
  ...
  shortcuts.forEach((settingKeys, shortcutValue) => {
    if (settingKeys.length > 1) {
      const hasUnallowedOverlap = settingKeys.some((currentKey) => {
        const currentSetting = settingsMap[currentKey] as ShortcutSetting;
        const otherKeys = settingKeys.filter((k) => k !== currentKey);

        // If this setting doesn't have allowOverlap, it doesn't allow any overlaps
        if (!currentSetting.allowOverlap || currentSetting.allowOverlap.length === 0) {
          return true;
        }

        return otherKeys.some(
          (otherKey) => !currentSetting.allowOverlap!.includes(otherKey as ShortcutSettingKeys),
        );
      });
...
function SettingInputRenderer({ settingKey, setting, settingsMap }: SettingsInputProps) {
  ...
  switch (setting.type) {
        ...
        <div className="grid grid-cols-2 gap-2 mb-0">
          {checkboxGroupSetting.options.map((option) => (
            <div key={option.value} className="flex items-center gap-x-2">
              <Label htmlFor={`${settingKey}-${option.value}`}>{option.label}</Label>
              <Switch
    ...
    case "textarea": {
      ...
      return (
        <Textarea
          id={settingKey}
          value={getSettingValue(setting) as string}
          onChange={(e: { target: { value: string | boolean | string[] } }) => {
            setting.onChange?.(e.target.value);
          }}
    ...
    case "select":
      return (
        ...
        >
          <ContextMenu>
            <ContextMenuTrigger render={<SelectTrigger id={settingKey} className="w-48 mb-0" />}>
              <SelectValue placeholder="Select an option" />
            </ContextMenuTrigger>
            <ContextMenuContent>
              <ContextMenuItem
                onClick={() => setting.onChange?.(getDefaultSettingsValue(setting))}
              ...
              {setting.contextMenuItems && setting.contextMenuItems.length > 0 && (
                <>
                  <ContextMenuSeparator />
    ...
    case "radio":
      return (
        ...
        >
          {setting.options.map((option) => (
            <div key={option.value} className="flex items-center gap-x-2">
              <RadioGroupItem value={option.value} id={`${settingKey}-${option.value}`} />
              <Label htmlFor={`${settingKey}-${option.value}`}>{option.label}</Label>
            </div>
          ))}
        </RadioGroup>
      ...
      return (
        <div className="flex w-full flex-col gap-2 mb-0">
          <div className="flex items-center justify-between">
            ...
            <input
              className={cn("w-12 border-0 text-center font-medium")}
              type="number"
              ...
              onChange={(e) => {
                ...
                if (!isNaN(newValue)) {
                  setting.onChange?.(Math.min(newValue, sliderSetting.max).toString());
                }
              }}
            />
            <span
          ...
          <Slider
            ...
            onValueChange={(values) => {
              setting.onChange?.(
                typeof values === "number" ? values.toString() : values[0].toString(),
              );
            }}
          />
        </div>
    ...
    case "button": {
      ...
      if (!buttonSetting.confirmation) {
        return (
          <Button onClick={() => buttonSetting.onClick?.()} className="mb-0">
            {buttonSetting.label}
          </Button>
        );
      }
...
function ShortcutSettingInput({ settingKey, setting, settingsMap }: SettingsInputProps) {
    ...
    <div className="flex items-center gap-x-2">
      <Input
        ...
        className={cn("max-w-60 mb-0", {
          "border-destructive bg-destructive/20 focus-visible:ring-destructive": isDuplicate,
        })}
        readOnly
        onClick={(e) => {
...
function SettingsInputWrapper({ settingKey, setting, settingsMap, className }: SettingsInputProps) {
  return (
    <div className={cn("space-y-2", className)}>
      ...
      <SettingInputRenderer settingKey={settingKey} setting={setting} settingsMap={settingsMap} />
    </div>
  );
}

export function SettingsInput({ settingKey, setting, settingsMap, className }: SettingsInputProps) {
