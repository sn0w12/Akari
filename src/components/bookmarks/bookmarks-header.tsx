...
export default function BookmarksHeader() {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      startTransition(async () => {
        try {
          const results = await getBookmarkSearchResults(searchValue.trim());
  ...
  const handleSelect = (result: BookmarkResult) => {
    void router.navigate({ to: "/manga/$mangaId", params: { mangaId: result.mangaId } });
  };
    ...
    <div className="relative mb-4">
      <div className="flex flex-row gap-2 md:gap-4">
        ...
        <Autocomplete
          ...
          filter={null}
          items={searchResults}
          itemToStringValue={(item: unknown) => (item as BookmarkResult).title}
        ...
        >
          ...
          />
          {shouldRenderPopup && (
            <AutocompletePopup aria-busy={isPending || undefined} align="start">
              {status && (
