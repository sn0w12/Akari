...
export const ChaptersPopup: React.FC<ChaptersPopupProps> = ({
  ...
  return (
    <ResponsiveModal desktop="popover" open={open} onOpenChange={setOpen}>
      ...
      />
      <ResponsiveModalPopup align="end" side="bottom" dialogClassName="min-w-64 w-auto">
        <ResponsiveModalDrawerOnly>
          <ResponsiveModalHeader>
            <ResponsiveModalTitle className="text-center">{title}</ResponsiveModalTitle>
          </ResponsiveModalHeader>
        </ResponsiveModalDrawerOnly>
        <ResponsiveModalPanel>
          {open ? (
...
function ChaptersList({
  ...
  return (
    <div ref={parentRef} className="h-96 overflow-y-auto md:h-64" data-scrollbar-custom>
      {isLoading ? (
        <div className="space-y-2 py-2">
          {Array.from(
            {
              length: estimatedChapters < 10 ? estimatedChapters : 10,
            },
            (_, i) => i,
          ).map((i) => (
        ...
        <div className="py-4 text-center text-muted-foreground">No chapters available</div>
      )}
    </div>
  );
}
...
const ChapterRow = memo(function ChapterRow({
  ...
  return (
    <Link
      ...
      className={cn("block rounded p-2 text-sm transition-colors duration-100 hover:bg-accent", {
        "bg-accent-positive hover:bg-accent-positive/90 text-white": isLastRead,
      })}
    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <span
          className={cn("shrink-0 text-xs", isLastRead ? "text-white" : "text-muted-foreground")}
        >
