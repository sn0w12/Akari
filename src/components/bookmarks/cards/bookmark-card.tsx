        ...
        <div className="flex gap-2">
          ...
          <div className="flex min-w-0 flex-1 flex-col justify-between">
            ...
            <div>
              ...
              <SubTitle label="Latest" value={`Ch. ${bookmark.latestChapter.number}`} />
              <SubTitle
                label="Released"
                value={formatRelativeDate(bookmark.latestChapter.createdAt)}
              />
            </div>
            <ActionButton bookmark={bookmark} className="hidden md:flex" />
          </div>
        </div>
...
function ActionButton({ bookmark, className }: ActionButtonProps) {
  ...
  const { release, handlers, style } = useLongPress(() => setOpen(true), 500, {
    ...
    <ButtonGroup className={cn("w-full", className)}>
      {isCaughtUp ? (
        ...
        <ButtonLink
          ...
          {...handlers}
        >
          <p className="hidden md:inline">Read Latest • </p>Ch. {bookmark.latestChapter.number}
        </ButtonLink>
      ) : (
        <ButtonLink
          to="/manga/$mangaId/$scanlator/$subId"
        ...
        >
          ...
          <Badge variant="default" className="ml-1.5 text-xs group-hover:bg-primary/80">
