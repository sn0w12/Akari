...
import { formatForDisplay, Hotkey, RegisterableHotkey, useHeldKeys } from "@tanstack/react-hotkeys";
...
function Kbd({ children, isPressed }: { children: React.ReactNode; isPressed: boolean }) {
...
function ContextKeyboardShortcut({ keys, className = "" }: KeyboardShortcutProps) {
  ...
  return (
    <span className={cn("text-muted-foreground pointer-events-none flex gap-1 text-sm", className)}>
      {formatForDisplay(keys as Hotkey)
        .split("+")
        .map((key, index) => (
          <Kbd key={`${index}-${key}`} isPressed={formattedPressedKeys.includes(key)}>
