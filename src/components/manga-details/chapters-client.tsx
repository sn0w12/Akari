...
export function ChaptersSection({
  ...
  const [currentScanlatorId, setCurrentScanlatorId] = useState<number>(preferredScanlator);
  ...
  useEffect(() => {
    if (!data) return;
    const lastReadChapter = chapters.find((chapter) => chapter.id === data.id);
        ...
        {currentChapters?.map((chapter) => (
          ...
          >
            <CardContent className="p-4">
              <h3
                className={cn("font-semibold mb-2 line-clamp-2", {
