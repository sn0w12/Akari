...
export function ScoreDisplay({ mangaId, rating }: ScoreDisplayProps) {
  ...
  const [initialRating, setInitialRating] = useState<number>(() => Math.round(rating.average));
  const starRefs = useRef<(HTMLDivElement | null)[]>([]);

  const score = useMemo(() => Math.max(0, Math.min(5, rating.average / 2)), [rating.average]);
  ...
  const handleMouseMove = (index: number, event: React.MouseEvent<HTMLDivElement>) => {
    ...
    setHoverState((prev) => (prev?.index === index && prev.fill === fill ? prev : { index, fill }));
  };
  ...
  const getHoverFill = (index: number): HoverFill => {
    ...
    if (index === hoverState.index) return hoverState.fill === 0.5 ? "half" : "full";
    return "empty";
  };
  ...
  return (
    <Card className="flex w-full h-full flex-col items-center justify-center p-2 xl:p-4">
      <div className="flex flex-col items-center justify-center relative top-1 xl:top-2.5">
            ...
            return (
              <div
                ...
                onClick={() => {
                  if (!user) return;
                  if (hoverState) {
                    setInitialRating((hoverState.index + hoverState.fill) * 2);
                  } else {
        ...
        <p className="text-sm text-accent-foreground/70 h-5">{score.toFixed(1)} / 5</p>
      </div>
      <RateDialog
        mangaId={mangaId}
        rating={rating}
...
const ScoreStars = memo(function ScoreStars({
  ...
  const isFirstVisible =
    ...
    hoverFill === "half" ||
    hoverFill === "full";
  const isSecondVisible = index < fullStars || (index === fullStars && hoverFill === "full");
  ...
  return (
    <>
      ...
      />
      <StarHalf
        className={cn(starClasses, "absolute inset-0 text-ring -scale-x-100", {
          "text-primary": !isAnyHovered,
