...
export function MangaCommentList({
  ...
  const [sortOrder, setSortOrder] = useState<components["schemas"]["CommentSortOrder"]>("Upvoted");
  ...
  const { data: userVoteData } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await client.GET("/v2/comments/{id}/votes", {
        params: {
          path: {
            id: mangaId,
          },
      ...
      if (error) {
        throw new Error(error.data.message || "Error fetching search results");
      }
  ...
  const [comments, setComments] =
    useState<components["schemas"]["CommentWithRepliesResponse"][]>(initialComments);

  const { data: user } = useUser();
  const [currentPage, setCurrentPage] = useState(1);
  const queryClient = useQueryClient();

  const handleSortChange = (newSort: components["schemas"]["CommentSortOrder"]) => {
  ...
  const loadMoreMutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      void queryClient.invalidateQueries({ queryKey: [mangaId] });
    },
    onError: () => {
      toastManager.add({
  ...
  const handleLoadReplies = async (commentId: string): Promise<void> => {
    const { data, error } = await client.GET("/v2/comments/{commentId}/replies", {
      params: {
        path: {
          commentId: commentId,
        },
      ...
      const updateReplies = (
        ...
        return comments.map((comment) => {
          if (comment.id === commentId) {
            ...
            return {
              ...comment,
              replies: updateReplies(comment.replies, commentId, replies),
            };
          }
          return comment;
        });
  ...
  const handleVote = async (commentId: string, voteType: VoteType): Promise<void> => {
  ...
  const handleReply = async (
    ...
    setComments((prevComments) => insertReply(prevComments, parentId, data.data));
  ...
  const handleEdit = async (commentId: string, content: string): Promise<void> => {
    ...
    setComments((prevComments) => updateCommentContent(prevComments, commentId, content));
  ...
  const handleDelete = async (commentId: string): Promise<void> => {
    ...
    setComments((prevComments) => updateCommentContent(prevComments, commentId, "[deleted]"));
  ...
  return (
    <>
      <div className="flex flex-row justify-between my-2 pb-2 border-b" id="comments">
        <h2 className="text-2xl font-semibold">Comments</h2>
        <CommentSorting sort={sortOrder} onSortChange={handleSortChange} />
      </div>
      <CommentList
        comments={comments}
      ...
      {currentPage < totalPages && (
        <div className="flex flex-1 justify-center mt-4">
          <Button
            ...
            className="w-full"
          >
            {loadMoreMutation.isPending ? "Loading..." : "Load More Comments"}
          </Button>
        </div>
      )}
    </>
