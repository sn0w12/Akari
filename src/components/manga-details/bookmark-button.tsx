...
import { bookmarkManga, checkIfBookmarked, removeBookmark } from "@/lib/manga/bookmarks";
...
export function BookmarkButton({
  ...
  const buttonContent = (
    <div className="relative flex h-full w-full items-center justify-center">
      {fancyAnimationsEnabled ? (
        <>
          <Bookmark
            className={`transition-all duration-300 ease-in-out ${
              isBookmarked && hovered ? "-translate-x-7" : "translate-x-0"
            }`}
          />
          <span
        ...
        <>
          <Bookmark />
          <span>{isBookmarked ? (hovered ? "Remove" : "Bookmarked") : "Bookmark"}</span>
        </>
      )}
    </div>
  );
  ...
  const button = (
    <Button
      ...
      loading={loading || isBookmarked === null || isPending || isQueryLoading}
      onClick={handleClick}
