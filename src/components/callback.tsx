...
import { getSecondaryAccountById, SecondaryAccountId } from "@/lib/auth/secondary-accounts";
...
const CallbackPage = () => {
  ...
  useEffect(() => {
    ...
    const getToken = async (accountId: string) => {
      const account = getSecondaryAccountById(accountId as SecondaryAccountId);
      if (!account) {
    ...
    const processCallback = async () => {
      try {
        const account = getSecondaryAccountById(provider as SecondaryAccountId);
        if (!account) {
          dispatch({
            type: "ERROR",
            error: {
        ...
        const cacheStorage = StorageManager.get("secondaryAccountCache", { accountId: provider });
    ...
    <div className="flex min-h-[100dvh] flex-col items-center justify-center bg-background">
      {error ? (
        ...
        <Card className="p-6 space-y-4 text-center">
          <p className={`${success ? "text-green-500" : "text-muted-foreground"}`}>
            {success ? "Success!" : "Exchanging authorization code for access token..."}
          </p>
        </Card>
      )}
    </div>
