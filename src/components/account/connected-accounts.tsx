  ...
  const handleLogout = async (account: SecondaryAccount) => {
    ...
    if (success) {
      setValidAccounts((accounts) =>
        accounts.map((acc) => (acc.id === account.id ? { ...acc, valid: false } : acc)),
      );
    }
  };
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Link external accounts to import and sync your manga collection
        </CardDescription>
      </CardHeader>
      <CardPanel>
        <Card>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  >
                    <span className="font-semibold">{account.id.toUpperCase()}</span>
                  </div>
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h3 className="font-medium text-foreground">
                        {accountUrl ? (
                          <a href={accountUrl} target="_blank" className="hover:underline">
                            {account.name}
                          </a>
                        ) : (
                          account.name
                      ...
                      {isValid && accountName && <Badge className="w-fit">{accountName}</Badge>}
                    </div>
                    <p className="text-sm text-muted-foreground">
                      {isValid ? "Account successfully linked" : "Not connected"}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {isValid ? (
                    <>
                      <ButtonLink
                        to={`/sync/${account.id}` as "/sync/ani" | "/sync/mal"}
                        variant="default"
