...
export function DesktopHeader({ notification }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    const modifiedSegments = pathSegments.map((segment) => overrides[segment] || segment);
    queueMicrotask(() => {
  ...
  return (
    <header className={cn(`z-50 bg-sidebar border-b-0 h-10`, borderClass)}>
      <div className="py-1 pr-4 md:pr-6.5 pl-11 mx-auto flex items-center justify-between">
        ...
        {user?.banned ? (
          <Badge variant="destructive" className="text-xs font-bold px-2 h-5 ml-2">
            Banned
          </Badge>
        ) : null}
        {inPreview ? (
          <Badge variant="warning" className="text-xs font-bold px-2 h-5 ml-2">
            Preview
          </Badge>
        ) : null}
        <div className={`flex h-full items-center pr-2 transition-all pl-4 md:pl-2`}>
          <Breadcrumb>
            <BreadcrumbList>
              {segments.map((segment, index) => (
                <React.Fragment key={originalSegments.slice(0, index + 1).join("/")}>
                  ...
                  <BreadcrumbItem>
                    {index != 0 ? (
                      <BreadcrumbLink to={`/${originalSegments.slice(0, index + 1).join("/")}`}>
                        {getSegmentDisplayName(segment)}
                      </BreadcrumbLink>
                    ) : (
                      <span>{getSegmentDisplayName(segment)}</span>
                    )}
                  </BreadcrumbItem>
                </React.Fragment>
              ))}
