...
## Step 3: Convert Layout â†’ Root Route
...
## Step 4: Convert Pages â†’ File Routes
...
## Step 5: Convert Server Actions â†’ Server Functions
...
```tsx
// app/posts/page.tsx (Server Component â€” server-only by default)
...
## Step 7: Convert API Routes â†’ Server Routes
...
```tsx
// src/start.ts â€” must be manually created
...
```tsx
// WRONG â€” treating component as server-only (Next.js habit)
function PostsPage() {
  const posts = await db.posts.findMany() // fails on client
  return <div>{posts.map(...)}</div>
}

// CORRECT â€” use server function + loader
const getPosts = createServerFn({ method: 'GET' }).handler(async () => {
  return db.posts.findMany()
})
...
```tsx
// WRONG â€” "use server" is Next.js/React pattern
'use server'
export async function myAction() { ... }

// CORRECT â€” use createServerFn
...
```tsx
// WRONG â€” Next.js pattern
<Link to={`/posts/${post.id}`}>View</Link>

// CORRECT â€” TanStack Router pattern
<Link to="/posts/$postId" params={{ postId: post.id }}>View</Link>
```

## Cross-References

- [react-start](../../react-start/SKILL.md) â€” full React Start setup
- [start-core/server-functions](../../../../start-client-core/skills/start-core/server-functions/SKILL.md) â€” server function patterns
- [start-core/execution-model](../../../../start-client-core/skills/start-core/execution-model/SKILL.md) â€” isomorphic execution
